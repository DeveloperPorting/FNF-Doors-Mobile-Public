...
class DoorsOption extends FlxSpriteGroup
{
	...
	#if mobile
	public var virtualPad:mobile.flixel.FlxVirtualPad;
	#end

	public function new(x:Float, y:Float, category:String, commonOption:CommonDoorsOption #if mobile, ?virtualPad:mobile.flixel.FlxVirtualPad #end)
	{
		super(x, y);
	...
	private function createBasicUI()
	{
		...
		descText = new FlxText(titleText.x + titleText.width + 20, 12, 962 - (titleText.x + titleText.width + 20),
			'|:|desc|:|options/${category}/${name}'.replace("\n", " "), 32);
	...
	override function update(elapsed:Float)
	{
		...
		if (optionType == DoorsOptionType.BUTTON)
		{
			...
			if (Controls.instance.ACCEPT || (FlxG.mouse.overlaps(bg) && FlxG.mouse.justPressed))
			{
				triggerButton();
			}
		}
	...
	{
		return leftSelector.isHovered
			&& FlxG.mouse.pressed
			|| (isSelected && (Controls.instance.UI_LEFT #if mobile || virtualPad != null && virtualPad.buttonLeft.pressed #end)); // resumo oq acontecia, era q por cuasa de um erro de logica a parte do mobile n era lida corretamente: vilgax
	}

	private function checkRightInput():Bool
	{
		return rightSelector.isHovered
			&& FlxG.mouse.pressed
	...
	{
		if (Controls.instance.UI_LEFT_P #if mobile || virtualPad != null && virtualPad.buttonLeft.justPressed #end)
		{
			whichSelected = "l";
			changeBgSpr(true);
		}
		else if (Controls.instance.UI_RIGHT_P #if mobile || virtualPad != null && virtualPad.buttonRight.justPressed #end)
		{
			whichSelected = "r";
			changeBgSpr(true);
		}
	...
	private function createButtonUI()
	{
		...
		descText = new FlxText(titleText.x + titleText.width + 20, 12, 1210 - (titleText.x + titleText.width + 20),
			'|:|desc|:|options/${category}/${name}'.replace("\n", " "), 32);
		descText.setFormat(FONT, 20, 0xFFFEDEBF, LEFT, OUTLINE, 0xFF452D25);
		descText.alpha = 0.8;
		descText.antialiasing = ClientPrefs.globalAntialiasing;
